...
async function scanContent(opts) {
  ...
  const deps = imports.filter((imp) => {
    const stmt = opts.content.slice(imp.ss, imp.se);
    return imp.d > -1 || /^import\s+[a-zA-Z_$][\w_$]*(\s+from|\s*,)/.test(stmt) || /^export\s+([a-zA-Z_$][\w_$]*\s+from|\*)/.test(stmt) || /(?<!type\s+){(\s*(?!type)[a-zA-Z_$]|.*,\s*(?!type)[a-zA-Z_$])/.test(stmt);
  }).map((imp) => {
