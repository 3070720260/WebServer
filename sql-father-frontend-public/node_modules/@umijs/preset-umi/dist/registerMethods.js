...
var registerMethods_default = (api) => {
  ...
  api.registerMethod({
    name: "writeTmpFile",
    fn(opts) {
      (0, import_assert.default)(api.service.stage >= api.ServiceStage.runCommand, `api.writeTmpFile() should not execute in register stage.`);
      const absPath = (0, import_path.join)(api.paths.absTmpPath, this.plugin.key && !opts.noPluginDir ? `plugin-${this.plugin.key}` : "", opts.path);
      import_utils.fsExtra.mkdirpSync((0, import_path.dirname)(absPath));
      let content = opts.content;
      if (!content) {
        (0, import_assert.default)(!opts.tplPath || (0, import_fs.existsSync)(opts.tplPath) && (0, import_fs.statSync)(opts.tplPath).isFile(), `opts.tplPath does not exists or is not a file.`);
        ...
        (0, import_assert.default)(import_utils.lodash.isPlainObject(opts.context), `opts.context must be plain object.`);
      ...
      if (api.appData.vite && isJsFile) {
        content = (0, import_transformIEAR.default)({ content, path: absPath }, api);
      }
