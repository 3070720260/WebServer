...
async function getRoutes(opts) {
  ...
  if (opts.api.config.routes) {
    routes = (0, import_core.getConfigRoutes)({
      ...
      onResolveComponent(component) {
        ...
        component = (0, import_utils.winPath)(import_utils.resolve.sync(localPath(component), {
          basedir: opts.api.paths.absPagesPath,
          extensions: [".js", ".jsx", ".tsx", ".ts", ".vue"]
        }));
        component = component.replace((0, import_utils.winPath)(`${opts.api.paths.absSrcPath}/`), "@/");
        return component;
      }
    });
  } else {
  ...
  for (const id of Object.keys(routes)) {
    if (routes[id].file) {
      ...
      if (opts.api.config.ssr || opts.api.config.clientLoader) {
        routes[id].__exports = isJSFile && (0, import_fs.existsSync)(file) ? await (0, import_getModuleExports.getModuleExports)({
          file
        }) : [];
      }
      if (opts.api.config.ssr) {
        routes[id].hasServerLoader = routes[id].__exports.includes("serverLoader");
      }
      if (opts.api.config.clientLoader) {
        routes[id].__hasClientLoader = routes[id].__exports.includes("clientLoader");
  ...
  const layouts = (await opts.api.applyPlugins({
    ...
    layout.file = layout.file.replace(new RegExp(`^${(0, import_utils.winPath)(absSrcPath)}`), "@");
    return layout;
  });
...
async function getRouteComponents(opts) {
  const imports = Object.keys(opts.routes).map((key) => {
    ...
    if (route.hasClientLoader) {
      route.file = (0, import_path.join)(opts.api.paths.absTmpPath, "pages", route.id.replace(/[\/\-]/g, "_") + ".js");
    }
    if (route.file.startsWith("(")) {
      return useSuspense ? `'${key}': React.lazy(
