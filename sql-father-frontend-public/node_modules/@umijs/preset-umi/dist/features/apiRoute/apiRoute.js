...
var apiRoute_default = (api) => {
  ...
  api.describe({
    ...
    enableBy: () => {
      ...
      if (!config.platform) {
        import_utils.logger.warn("Please set config.apiRoute.platform to enable API route feature!");
        return false;
      }
      platform = getPlatform(config.platform);
      if (!platform) {
        import_utils.logger.warn("There is an invalid value of config.apiRoute.platform: " + config.platform + ", so API route feature will not be enabled!");
        return false;
      }
      if (platform !== "vercel" /* Vercel */) {
        import_utils.logger.warn("Current version of Umi only supports deploying API routes to Vercel, so API route feature will not be enabled!");
        return false;
      }
      if (!import_fs.default.existsSync((0, import_path.join)(api.paths.cwd, "vercel.json"))) {
        import_utils.logger.warn("You have enabled the API route feature, but there is no vercel.json file in your work directory! Automatically creating a vercel.json file ...");
        import_fs.default.writeFileSync((0, import_path.join)(api.paths.cwd, "vercel.json"), JSON.stringify({ build: { env: { ENABLE_FILE_SYSTEM_API: "1" } } }, null, 2));
      }
      return true;
    }
  });
    ...
    apiRoutes.map((apiRoute) => {
      api.writeTmpFile({
        ...
        context: {
          ...
          handlerPath: (0, import_utils.winPath)((0, import_path.join)(api.paths.absSrcPath, "api", apiRoute.file)),
  ...
  api.addBeforeMiddlewares(() => [
    async (req, res, next) => {
      if (req.path.startsWith("/api")) {
        ...
        await require((0, import_path.join)(api.paths.cwd, import_constants2.OUTPUT_PATH, matchedApiRoute.route.file).replace(".ts", ".js")).default(req, res);
        return;
      }
      next();
    }
  ...
  api.onBeforeCompiler(async () => {
    if (api.env === "development") {
      (0, import_watch.watch)({
        ...
        onChange(e, p) {
          if (e === "add") {
            import_utils.logger.event(`New API route ${(0, import_path.basename)(p)} detected, compiling ...`);
            api.restartServer();
            return;
          }
          if (e === "unlink") {
            import_utils.logger.event(`API route ${(0, import_path.basename)(p)} has been removed, compiling ...`);
            api.restartServer();
            return;
          }
        }
    ...
    switch (platform) {
      ...
      case "netlify" /* Netlify */:
        ...
        return;
      case "worker" /* Worker */:
        import_utils.logger.error("API routes bundle failed: Cloudflare Worker is not supported yet!");
        return;
      default:
