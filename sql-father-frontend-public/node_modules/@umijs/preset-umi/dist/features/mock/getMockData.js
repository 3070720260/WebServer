...
function getMockData(opts) {
  ...
  const ret = [import_constants.MOCK_FILE_GLOB, ...opts.mockConfig.include || []].reduce((memo, pattern) => {
    memo.push(...import_utils.glob.sync(pattern, {
      cwd: opts.cwd,
...
${e.message}`);
    ...
    for (const key of Object.keys(obj)) {
      ...
      (0, import_assert.default)(import_utils.lodash.isArray(mock.handler) || import_utils.lodash.isPlainObject(mock.handler) || typeof mock.handler === "function", `Mock handler must be function or array or object, but got ${typeof mock.handler} for ${mock.method} in ${mock.file}`);
      if (memo[id]) {
        import_utils.logger.warn(`${id} is duplicated in ${normalizeMockFile(mockFile)} and ${normalizeMockFile(memo[id].file)}`);
      }
      memo[id] = mock;
    }
    return memo;
...
function parseKey(key) {
  ...
  if (len === 1) {
    ...
    (0, import_assert.default)(import_constants.VALID_METHODS.includes(upperCaseMethod), `method ${method} is not supported`);
