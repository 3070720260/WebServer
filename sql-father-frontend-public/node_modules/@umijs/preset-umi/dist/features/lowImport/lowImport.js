...
var lowImport_default = (api) => {
  ...
  api.onStart(() => {
    const dts = api.appData.lowImport.map((lib) => {
      if (lib.withObj) {
        const memberDts = (lib.members || []).map((member) => `${member}: typeof import('${lib.importFrom}')['${member}'],`).join("\n");
...
${memberDts}
        ...
        return (lib.members || []).map((member) => `const ${member}: typeof import('${lib.importFrom}')['${member}'];`).join("\n");
      }
    });
