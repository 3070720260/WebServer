...
var ESMIService = class {
  ...
  setCache(key, data) {
    ...
    import_fs.default.writeFileSync(import_path.default.join(this.cacheDir, "importmap.json"), JSON.stringify(this.cache, null, 2));
  }
  async build(data) {
    return import_utils.axios.post(`${this.cdnOrigin}/api/v1/esm/build`, data).then((a) => {
      var _a;
  ...
  async getImportmap(data) {
    ...
    const next = () => new Promise((resolve) => setTimeout(() => resolve(deferrer()), 2e3));
    const deferrer = () => {
      return import_utils.axios.get(`${this.cdnOrigin}/api/v1/esm/importmap/${ticketId}`).then((res) => {
        if (res.data.success) {
          this.setCache(cacheKey, res.data.data);
          import_utils.logger.info(`Done, took ${((+new Date() - stamp) / 1e3).toFixed(1)}s`);
