    ...
    async transform(code, id) {
      if (id.endsWith(".svg")) {
        ...
        if (svgr) {
          ...
          const svgrCode = await (0, import_core.transform)(svgFile, __spreadProps(__spreadValues({
            icon: true,
            svgoConfig: __spreadValues({}, svgo || {})
          }, svgr), {
            svgo: !!svgo
          }), { componentName: "ReactComponent" });
          componentCode = svgrCode.replace("export default ReactComponent", "export { ReactComponent }") + "\n" + code;
        }
        const result = await (0, import_esbuild.transform)(componentCode, __spreadValues({
          loader: "jsx",
          sourcefile: id,
          sourcemap: true
        }, transformOptions));
