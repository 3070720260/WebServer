...
function getSchemas() {
  return {
    ...
    copy: (Joi) => Joi.array().items(Joi.alternatives().try(Joi.string(), Joi.object().keys({
      from: Joi.string(),
      to: Joi.string()
    }))),
    ...
    jsMinifier: (Joi) => Joi.alternatives().try(Joi.string().valid(import_types.JSMinifier.esbuild, import_types.JSMinifier.terser), Joi.boolean()),
